...
export default function Productpageinfo({ product, cartlist, setcartlist }) {
    ...
    // تحقق إذا كان المنتج يحتوي على ألوان
    const hasColors = productData.colors_sizes && productData.colors_sizes.length > 0;

    // تحقق إذا كان المنتج يحتوي على مقاسات (للون المحدد أو بشكل عام)
    const hasSizes = () => {
        if (!hasColors) return false;

        // إذا كان هناك لون محدد، تحقق من مقاساته
        if (selectedColor) {
            return selectedColor.sizesAtColor && selectedColor.sizesAtColor.length > 0;
        }

        // إذا لم يكن هناك لون محدد، تحقق إذا كان أي لون يحتوي على مقاسات
    ...
    // معالجة نقر إضافة المنتج
    function addproductclicked() {
        // إذا كان المنتج يحتوي على ألوان ولكن لم يتم اختيار لون
        if (hasColors && !selectedColor) {
            alert(t('selectColorFirst'));
            return;
        }

        // إذا كان المنتج يحتوي على مقاسات ولكن لم يتم اختيار مقاس
        ...
        const newProduct = {
            ...
            size: selectedSize || "",
            quantity: 1,
            // إنشاء معرف فريد بناءً على السيناريو
    ...
    // تحديد السعر الحالي عند تحميل المنتج
    useEffect(() => {
        // إذا كان المنتج يحتوي على ألوان، استخدم سعر أول لون كافتراضي
        if (hasColors && productData.colors_sizes[0].price) {
            setCurrentPrice(productData.colors_sizes[0].price);
        } else {
            // إذا لم يكن هناك ألوان، استخدم السعر العام
    ...
    const handleColorChange = (colorItem) => {
        ...
        // إذا كان المنتج يحتوي على مقاسات لهذا اللون
        if (colorItem.sizesAtColor && colorItem.sizesAtColor.length > 0) {
            // إذا كان المقاس المحدد غير متوفر في هذا اللون، ألغِ التحديد
            ...
            // إذا لم يكن هناك مقاسات، قم بتعيين isCurrentSizeValid إلى true
            setIsCurrentSizeValid(true);
            setSelectedSize(null); // لا حاجة لمقاس
        }
    };
    ...
    const isButtonDisabled = () => {
        ...
        // التحقق من صحة المقاس بالنسبة للون المحدد
        if (selectedColor && selectedSize) {
        ...
        <div className="container-2" dir={direction} lang={currentLanguage}>
            <div className="product-page-info">
                ...
                <div className="product-page-des">
                    ...
                    {/* قسم الألوان - يظهر فقط إذا كان المنتج يحتوي على ألوان */}
                    {hasColors && (
                        <div className="product-page-color-size" dir="auto">
                            <div className="product-page-color">
                                <p>{t('colors')}</p>
                                {productData.colors_sizes.map((item, index) => (
                                    <div key={index} className="color-option">
                                        ...
                                        >
                                            {item.colorNameInArabic}
                                            {/* {item.price && item.price !== productData.global_price &&
                                                ` (${item.price})`} */}
                                        </label>
                                    </div>
                                ))}
                            ...
                            {/* قسم المقاسات - يظهر فقط إذا كان المنتج يحتوي على مقاسات */}
                            {hasSizes() && (
                                <div className="product-page-size">
                                    <p>{t('sizes')}</p>
                                    {selectedColor ? (
